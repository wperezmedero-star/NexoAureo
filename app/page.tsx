...
export default function Home() {
  ...
  return <main className="site-shell">
    ...
    {modal && <div className="modal-backdrop" role="presentation" onMouseDown={() => setModal(false)}><div className="login-modal secure-login-modal" role="dialog" aria-modal="true" aria-labelledby="access-title" onMouseDown={(e) => e.stopPropagation()}><button className="modal-close" aria-label="Cerrar" onClick={() => setModal(false)}>×</button><Logo animate /><div className="secure-access-icon" aria-hidden="true">◇</div><h2 id="access-title">Acceso privado de William</h2><p>El sitio informativo permanece público. El CRM, las entrevistas, el historial y las herramientas internas requieren la cuenta autorizada.</p>{(auth.status === "denied" || auth.status === "error") && <div className="access-alert" role="alert">{auth.message}</div>}{auth.status === "authorized" ? <button className="primary-button login-submit secure-login-action" type="button" onClick={() => openPrivateSpace(pendingView)}>Abrir espacio protegido</button> : <a className="primary-button login-submit secure-login-action" href={signInHref}>Continuar con ChatGPT</a>}<div className="secure-access-points"><span>✓ Identidad administrada por la plataforma</span><span>✓ Solo la cuenta autorizada</span><span>✓ API y datos bloqueados en el servidor</span></div><small className="demo-note">NexoÁureo no recibe ni guarda tu contraseña. Safari puede ofrecer Face ID cuando la cuenta dispone de una passkey.</small></div></div>}
  </main>;
}
